...
#[test]
fn test_local_ijk_to_cell_vs_c_intermediate_path_step1() {
  let start_h3 = START_H3_FAIL_CASE; // 0x855943cbfffffff
  let local_h3_ijk_plus_for_n1 = H3_IJK_PLUS_N1_RUST; // {2,0,1} (derived from axial {-1,-1,2})

  let expected_c_path_n1_cell = C_PATH_N1; // 0x855943cffffffff

  let mut rust_generated_cell_n1 = H3_NULL;
  let res = local_ijk_to_cell(start_h3, &local_h3_ijk_plus_for_n1, &mut rust_generated_cell_n1);

  assert!(res.is_ok(), "local_ijk_to_cell failed: {:?}", res.err());

  eprintln!("TEST_LIJ2C_VS_C: Start H3: {:x}", start_h3.0);
  eprintln!("TEST_LIJ2C_VS_C: Local IJK+ (for n=1): {:?}", local_h3_ijk_plus_for_n1);
  eprintln!(
    "TEST_LIJ2C_VS_C: Rust generated cell for n=1: {:x}",
    rust_generated_cell_n1.0
  );
  eprintln!(
    "TEST_LIJ2C_VS_C: Expected C cell for n=1: {:x}",
    expected_c_path_n1_cell.0
  );

  assert_eq!(
    rust_generated_cell_n1, expected_c_path_n1_cell,
    "Rust's local_ijk_to_cell for n=1 input does not match C's path[1]"
  );
}

#[test]
fn test_local_ijk_to_cell_vs_c_intermediate_path_step2() {
  let start_h3 = START_H3_FAIL_CASE; // 0x855943cbfffffff
  let local_h3_ijk_plus_for_n2 = H3_IJK_PLUS_N2_RUST; // {4,0,2} (derived from axial {-2,-2,4})

  let expected_c_path_n2_cell = C_PATH_N2; // 0x8559431bfffffff

  let mut rust_generated_cell_n2 = H3_NULL;
  let res = local_ijk_to_cell(start_h3, &local_h3_ijk_plus_for_n2, &mut rust_generated_cell_n2);

  assert!(res.is_ok(), "local_ijk_to_cell failed: {:?}", res.err());

  eprintln!("TEST_LIJ2C_VS_C: Start H3: {:x}", start_h3.0);
  eprintln!("TEST_LIJ2C_VS_C: Local IJK+ (for n=2): {:?}", local_h3_ijk_plus_for_n2);
  eprintln!(
    "TEST_LIJ2C_VS_C: Rust generated cell for n=2: {:x}",
    rust_generated_cell_n2.0
  );
  eprintln!(
    "TEST_LIJ2C_VS_C: Expected C cell for n=2: {:x}",
    expected_c_path_n2_cell.0
  );

  assert_eq!(
    rust_generated_cell_n2, expected_c_path_n2_cell,
    "Rust's local_ijk_to_cell for n=2 input does not match C's path[2]"
  );
}

const TEST_ORIGIN_H3: H3Index = H3Index(0x855943cbfffffff);

// For path[1] (n=1 in grid_path_cells loop)
const LOCAL_IJK_N1: CoordIJK = CoordIJK { i: 0, j: 0, k: 3 };
const EXPECTED_H3_N1_C_VERSION: H3Index = H3Index(0x855943cfffffff);
const CURRENT_RUST_H3_N1: H3Index = H3Index(0x85594303fffffff); // From logs

// For path[2] (n=2 in grid_path_cells loop)
const LOCAL_IJK_N2: CoordIJK = CoordIJK { i: 0, j: 0, k: 6 };
const EXPECTED_H3_N2_C_VERSION: H3Index = H3Index(0x8559431bfffffff);
const CURRENT_RUST_H3_N2: H3Index = H3Index(0x85594337fffffff); // From logs

#[test]
fn test_local_ijk_to_cell_matches_c_path_step1() {
  let mut result_h3 = H3_NULL;
  let res = local_ijk_to_cell(TEST_ORIGIN_H3, &LOCAL_IJK_N1, &mut result_h3);

  eprintln!("--- Test: local_ijk_to_cell_matches_c_path_step1 ---");
  eprintln!("Origin H3: {:x}", TEST_ORIGIN_H3.0);
  eprintln!("Local IJK for n=1: {:?}", LOCAL_IJK_N1);
  eprintln!("local_ijk_to_cell result: {:?}", res);
  eprintln!("Output H3: {:x}", result_h3.0);
  eprintln!("Expected C H3 for n=1: {:x}", EXPECTED_H3_N1_C_VERSION.0);
  eprintln!("Current Rust H3 for n=1 (from path log): {:x}", CURRENT_RUST_H3_N1.0);

  assert!(res.is_ok(), "local_ijk_to_cell failed for step 1: {:?}", res.err());
  assert_eq!(
    result_h3, EXPECTED_H3_N1_C_VERSION,
    "local_ijk_to_cell output for step 1 does not match C path's N1_c. Rust produced {:x}",
    result_h3.0
  );
}

#[test]
fn test_local_ijk_to_cell_matches_c_path_step2() {
  let mut result_h3 = H3_NULL;
  let res = local_ijk_to_cell(TEST_ORIGIN_H3, &LOCAL_IJK_N2, &mut result_h3);

  eprintln!("--- Test: local_ijk_to_cell_matches_c_path_step2 ---");
  eprintln!("Origin H3: {:x}", TEST_ORIGIN_H3.0);
  eprintln!("Local IJK for n=2: {:?}", LOCAL_IJK_N2);
  eprintln!("local_ijk_to_cell result: {:?}", res);
  eprintln!("Output H3: {:x}", result_h3.0);
  eprintln!("Expected C H3 for n=2: {:x}", EXPECTED_H3_N2_C_VERSION.0);
  eprintln!("Current Rust H3 for n=2 (from path log): {:x}", CURRENT_RUST_H3_N2.0);

  assert!(res.is_ok(), "local_ijk_to_cell failed for step 2: {:?}", res.err());
  assert_eq!(
    result_h3, EXPECTED_H3_N2_C_VERSION,
    "local_ijk_to_cell output for step 2 does not match C path's N2_c. Rust produced {:x}",
    result_h3.0
  );
}
